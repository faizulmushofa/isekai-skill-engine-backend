...
export class JwtService {
  private readonly accessTokenSecret = process.env.JWT_ACCESS_SECRET || 'access_secret_123';
  private readonly refreshTokenSecret = process.env.JWT_REFRESH_SECRET || 'refresh_secret_123';
  ...
  verifyAccessToken(token: string): { userId: string } {
    try {
      ...
      throw new UnauthorizedException('Access token tidak valid atau telah kedaluwarsa');
    }
  }
